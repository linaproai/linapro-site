...
### Requirement:插件安装请求必须支持可选的模拟数据加载
...
### Requirement:模拟 SQL 和记账写入必须共享同一数据库事务
...
### Requirement:模拟阶段失败必须返回可操作的失败信息
...
#### Scenario:有模拟数据的插件显示正面可用性
- **当** 插件列表行有 `hasMockData=true` 时
- **则** 模拟数据列显示正面可用性标签
- **且** 插件名称不包含额外的模拟数据标记

#### Scenario:无模拟数据的插件显示负面可用性
...
### Requirement:卸载确认必须强调插件自有数据清理风险和硬删除语义
