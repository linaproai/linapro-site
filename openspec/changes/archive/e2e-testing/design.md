...
默认 E2E 套件使用 Chromium headless 模式，因此开发环境初始化下载 Playwright Chromium headless shell。Linux 仍通过`--with-deps`安装系统依赖。需要 headed/debug 的开发者显式安装完整 Chromium 浏览器，避免默认初始化为少数调试场景承担下载成本。
