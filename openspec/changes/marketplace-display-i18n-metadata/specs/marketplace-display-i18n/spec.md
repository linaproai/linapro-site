...
### Requirement: 列表与详情按请求语言投影展示字段

系统 SHALL 在市场目录列表、「我的插件」列表、管理列表与插件详情中，按当前请求语言返回已本地化的 `name` 与 `summary`。回退顺序 MUST 为：请求 locale → 插件声明的默认 locale → `en-US` → 身份表 fallback 字段。
...
默认管理工作台中的市场「我的插件」页与目录列表页 MUST 在当前语言变更后重新请求列表接口，以使 `name`/`summary` 与新的请求语言一致。
