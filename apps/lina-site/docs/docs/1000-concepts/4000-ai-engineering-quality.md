...
- **架构设计约束**：明确项目定位与`lina-core`核心宿主边界，防止核心域与工作台展示结构强绑定，保持核心领域契约的稳定性。
- **模块设计规范**：业务模块支持按需禁用、前端联动隐藏；数据权限必须在查询层显式注入，不允许内存层过滤替代；源码插件目录结构严格固定。
- **接口设计约束**：强制`RESTful`语义，禁止动作化路径命名，全仓库资源路径风格保持一致等规范。
- **后端代码质量规范**：运行期依赖必须显式注入，错误必须通过`bizerr`封装，日志沿调用链传递`ctx`，缓存在集群模式下必须跨实例协调。
...
在`AI`时代，速度是起点，不是终点。`LinaPro`追求的是：在`AI`全速前进的同时，质量不掉队。
