...
title: 'AI工程质量保障'
hide_title: true
description: '本文系统梳理当前将 AI 引入软件开发时面临的工程管理挑战，详细介绍 LinaPro 如何从 SDD 规范驱动工作流、两层项目规范约束、接口抽象与防泄漏契约、以及高密度测试覆盖（测试代码占总量 39%）四个维度构建完整的 AI 工程质量保障体系，并说明业务开发团队在使用框架时如何直接继承这套质量能力，无需从零搭建即可获得系统性的工程质量保障。'
keywords:
  - AI工程质量
  - 工程质量保障
  - AI代码质量
...
| **变更可追溯** | 代码提交历史难以反映设计意图 | 每次变更有对应的 proposal 和 design |
| **跨会话一致性** | AI 每次对话都从零开始理解 | 规范文档提供跨会话的稳定上下文锚点 |
| **架构漂移检测** | 只能靠代码评审发现 | E2E 测试作为硬性门禁，漂移即失败 |

`LinaPro`通过`OpenSpec`工具实现`SDD`工作流，工具提供`/opsx:explore`、`/opsx:propose`、`/opsx:apply`和`/opsx:archive`四个阶段命令，支持完整的五阶段研发闭环。详情参见：[AI规范驱动开发](/docs/spec-driven-development)。
...
#### 常驻约束文件：AGENTS.md / CLAUDE.md

`AGENTS.md`（同时作为`CLAUDE.md`的符号链接）是`LinaPro`项目中`AI`编码代理在每次会话开始时必须优先读取的**项目级规范文件**。与`openspec/specs/`中的能力域规范不同，`AGENTS.md`/`CLAUDE.md`是一套**常驻的、跨迭代的全局约束**，无论当前迭代涉及什么功能，这套约束都始终有效。

`AGENTS.md`/`CLAUDE.md`覆盖的核心约束维度包括：

- **架构设计约束**：明确项目定位（面向可持续交付的`AI`原生全栈框架），规定`lina-core`核心宿主边界——不得与工作台展示结构强绑定，工作台层变化应在适配层解决，不得下沉到核心领域契约。

- **模块设计规范**：要求所有业务模块支持按需禁用，禁用时前端联动隐藏相关`UI`元素；数据权限接入必须显式评估，读取类接口在数据库查询阶段注入过滤，不允许内存层过滤代替查询层隔离；源码插件目录结构严格固定，不允许将业务`service`放在`backend/service/`等非规范位置。

- **接口设计约束**：强制遵循`RESTful`语义，`GET`/`POST`/`PUT`/`DELETE`与操作语义严格对应；禁止动作化路径命名，要求资源化路径风格全仓库保持一致。

- **编译与测试门禁**：任何新增或修改`Go`生产代码的任务，在标记完成前必须运行覆盖变更包的`go test`烟测；涉及控制器构造、路由绑定或`API`接口签名变更时，还必须运行启动绑定包测试。不允许仅凭静态分析或历史记录认定后端变更可编译。

- **持续治理要求**：所有功能改动都必须评估对`i18n`的影响，明确判断翻译键是否需要新增、修改或删除；涉及缓存的设计必须区分单机部署与集群部署策略，`cluster.enabled=true`时禁止退化为仅当前节点可见的本地缓存；`bugfix`修复必须新增或更新能验证修复有效性的自动化测试，功能行为类缺陷不得无测试标记完成。
...
以后端一致性规范为例，该规范明确约束了诸如：

- 控制器的依赖必须通过构造函数显式注入，不得在请求路径中隐式创建服务实例
- 服务组件按职责拆分到独立文件，不允许大文件承载多个职责
- 数据库操作使用`DO`对象传递数据，不手工维护框架自动管理的时间字段
- 运行时错误返回`error`给调用方，不得在业务路径上使用`panic`替代错误处理

这些规范文件是`AI`进行代码生成时的上下文依据，也是代码评审时的对照标准。当一段`AI`生成的代码与规范不符时，问题可以被明确识别，而不是依赖审查者的主观判断。

#### 两层规范的分工

| 规范层次 | 文件 | 适用范围 | 更新时机 |
|---------|------|---------|---------|
| **常驻约束** | `AGENTS.md` / `CLAUDE.md` | 全项目、全迭代、所有`AI`会话 | 架构决策变化时修订 |
...
这正是`LinaPro`对"`AI`工程质量"这一命题给出的系统性回答：**不只是"写得快"，更是"写得对、写得稳、经得起验收"**。
