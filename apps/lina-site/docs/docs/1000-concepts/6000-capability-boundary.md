...
主框架`lina-core`提供的领域能力可以分为两类：**基础设施能力**和**插件扩展接口**。
...
这些能力被封装为稳定的服务适配器，向插件暴露。源码插件通过`pluginhost.Services`（内嵌`capability.Services`）直接调用领域能力方法；动态插件通过`plugin.yaml`声明`hostServices`，再经由`pluginbridge`的`host_call`机制访问。主框架不会为某个业务场景单独定义一个专用接口，而是提供可以组合使用的通用原语，由插件在自身逻辑中决定如何使用。
...
主框架将领域能力通过稳定的服务适配器接口向插件暴露，插件只需调用这些接口，无需了解主框架内部实现。源码插件通过`pluginhost.Services`访问主框架能力，该接口内嵌`capability.Services`并额外提供可信管理命令`Admin()`和租户过滤`TenantFilter()`；动态插件通过`pluginbridge`暴露的`host_call`机制访问主框架能力，运行时调用经过`hostServices`授权快照校验。

主框架的领域能力涵盖认证、缓存、配置、国际化、通知、租户、文件存储、数据记录、`AI`等多个服务域，并会随版本迭代持续扩充。插件可以根据实际需要选择性地使用这些能力，无需感知主框架内部服务的具体实现。
