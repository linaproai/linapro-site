...
定时任务是`lina-core`提供的持久化调度能力。任务定义、运行状态和执行日志保存在数据库中，宿主启动后会把宿主内置任务、插件声明任务和管理员创建的任务统一纳入调度。
...
| 类型 | 值 | 说明 | 适用场景 |
|------|----|------|----------|
| **处理器任务** | `handler` | 调用宿主或插件注册的`Go`处理器 | 数据清理、统计、同步、插件业务任务 |
| **Shell任务** | `shell` | 在宿主进程上下文执行系统命令 | 脚本任务、文件处理、运维辅助操作 |

处理器任务更适合业务逻辑，因为它可以复用宿主服务、数据库连接、权限上下文和插件治理信息。`Shell`任务具有宿主进程权限，生产环境应严格限制创建权限并设置合理超时。
...
宿主内置任务中也会使用带秒级语义的内部表达式，例如`# 17 3 * * *`用于把维护任务错开到固定秒点执行。管理员在工作台中创建任务时，应以界面支持的表达式和预览结果为准。
...
动态插件可以通过插件清单和`hostServices`授权声明自己的任务能力。宿主会把插件任务转换为统一的处理器引用，并在插件不可用、禁用或升级异常时暂停相关任务，避免调度到不可执行的目标。
...
`Shell`任务会以宿主服务进程的权限运行系统命令。生产环境使用时应遵守这些原则：
