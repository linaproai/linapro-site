...
这种分层保证了宿主始终稳定，租户业务规则可以通过官方插件或业务插件继续扩展。
...
- 单租户项目可以直接使用平台租户运行。
- 宿主基础表和插件表都可以保留`tenant_id`字段，为后续多租户演进留出空间。
- 平台请求是否绕过租户过滤由宿主策略控制，不应由插件自行硬编码。
- 启用`multi-tenant`插件后，用户可以进入具体租户上下文，也可以由平台管理员执行租户代管。

## 请求上下文

宿主通过`bizctx`向源码插件暴露只读请求快照。核心字段包括：
...
插件不需要访问宿主内部上下文对象，只通过`pluginservice`发布的契约读取必要信息。

## 租户过滤服务

宿主向源码插件发布`TenantFilterService`，用于对插件自有表追加租户过滤条件。默认过滤列名由`TenantFilterColumn`定义：
...
清单字段会被宿主启动一致性检查和插件治理流程使用。字段组合不合法时，宿主会在启动或插件扫描阶段暴露明确错误。
...
新租户是否自动开通某个插件，不由`plugin.yaml`直接决定，而是由宿主治理记录和`multi-tenant`插件的开通策略维护。
