...
宿主在响应前端菜单请求时，会将已启用插件声明的菜单与内置菜单合并，按当前用户权限过滤后一起返回给工作台。插件禁用后，对应菜单入口会自动从导航中消失，工作台无需为此单独修改代码。
...
开发者可以用自定义前端完全替换内置工作台，只要新的前端遵循宿主公开的`RESTful API`和权限模型，就能使用同一套后端能力。

工作台页面路径、宿主控制面`API`和插件`API`是三个独立边界：
...
主框架通过`Go Embed`管理自身静态资源，并通过声明式`public_assets`模型为插件托管可公开静态资源。源码插件和动态插件都可以在`plugin.yaml`根字段中声明可公开的资源目录，由宿主统一映射到`/x-assets/{plugin-id}/{version}/...`，例如：
...
主框架将基础能力通过稳定的服务适配器接口向插件暴露，插件只需调用这些接口，无需了解宿主内部实现。
...
**动态插件**通过`pluginbridge`暴露的`host_call`机制访问宿主能力，所有调用都经过授权快照校验。动态插件在`plugin.yaml`中声明`hostServices`申请所需的服务和方法，宿主安装时记录这份授权快照，运行时对每次`host_call`按快照校验服务、方法和资源边界：
...
| 服务 | 能力 |
|------|------|
| `runtime` | 运行时日志、插件作用域运行时状态读取、写入和删除，以及宿主时间、`UUID`、节点信息等运行时信息 |
| `cron` | 动态插件定时任务注册 |
...
**对主框架而言**，稳定的扩展接口意味着内部实现可以安全地演进——宿主可以重构`HostServices`的具体实现、优化插件治理流程、扩展新的基础能力，只要`pluginhost`和`pluginbridge`的公开契约保持稳定，所有已有插件就不会受到影响。

**对插件开发者而言**，明确的边界意味着可以放心地使用宿主提供的基础能力，无需了解其内部实现，也不必担心未来版本的宿主升级会破坏插件逻辑——只要插件严格通过稳定契约与主框架协作，版本兼容性就由主框架的接口稳定性来保障。
