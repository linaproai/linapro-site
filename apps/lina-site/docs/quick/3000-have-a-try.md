...
:::info 提示
`LinaPro`的开发工程中，用户可以不需要显式调用`OpenSpec`相关的斜杠命令，可以直接通过自然语言指令来驱动整个开发流程，`AI`会自动识别指令意图并执行相应的工作流操作。这里为了更好地演示工作流程，因此我们显式调用斜杠命令来展示每个阶段的操作。
:::
...
代码实现完成后，`AI`通常会关闭服务，并汇报工作完成。这时你可以手动启动服务，并对`AI`完成的工作内容进行查看和验收，执行以下指令：
...
接下来，你可以参考[扩展开发](../docs/3000-plugin-development/3000-plugin-development.md)文档深入了解源码插件和`WASM`动态插件的完整开发规范，或者查看[开发手册](../docs/2000-architecture/2000-architecture.md)了解框架的详细架构设计。
