...
title: '项目介绍'
hide_title: true
description: '本文全面介绍 LinaPro 的项目背景、技术架构与核心功能。LinaPro 是一款 AI 原生全栈框架，基于 Go + Vue 3 构建，内置双模式插件系统、完整的 RBAC 权限管理、定时任务调度、在线 API 文档、规范驱动的 OpenSpec 研发工作流和覆盖全生命周期的 AI 技能体系，帮助开发者和企业快速构建可持续交付的业务系统。'
keywords:
  - LinaPro
  - AI原生框架
  - 全栈框架
...
`LinaPro`是一款**面向可持续交付的 AI 原生全栈框架**，基于`Go + Vue 3`构建，把`AI`作为核心生产力，让团队从第一天起就能以`AI`作为主力工程师进行业务开发和持续交付。

## 项目介绍

`LinaPro`将后端服务、前端工作台、可插拔插件体系、规范驱动的`AI`研发工作流和丰富的`AI`技能体系融为一体，构成一套完整的全栈交付框架。框架内置开箱即用的默认管理工作台，覆盖了绝大多数业务系统所需的权限管理、系统配置、任务调度等基础能力，让团队把精力聚焦在业务本身，而非重复造轮子。

`LinaPro`的核心价值在于：通过规范驱动的`AI`原生研发工作流、覆盖全生命周期的`AI`技能体系、完整的插件扩展运行时以及前后端一体化的全栈设计，让`AI`始终在经过验证的基础上持续推进——而非在模糊需求上盲目输出。
