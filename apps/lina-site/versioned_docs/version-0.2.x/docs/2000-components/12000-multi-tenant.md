...
`LinaPro`把多租户能力拆成“主框架基础支撑”和“官方租户控制面”两层。主框架负责在请求上下文、插件服务契约和数据过滤中提供稳定基础；官方`multi-tenant`源码插件负责租户主体、成员关系、租户解析、租户代管和租户级插件治理。
...
| 层级 | 位置 | 职责 |
|------|------|------|
| **主框架基础能力** | `apps/lina-core` | 请求级`bizctx`、身份快照、`tenant_id`过滤接口、平台绕过策略、插件多租户元数据 |
