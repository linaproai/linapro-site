...
title: '主框架功能设计'
hide_title: true
description: '主框架在系统中的职责边界、运行时链路、启动加载流程、内置平台能力矩阵、插件扩展接缝以及与管理工作台、源码插件、WASM动态插件、PostgreSQL和Redis之间的协作关系，帮助开发者先建立整体认知，再进入配置、权限、接口文档、定时任务、国际化、多租户、插件系统和分布式架构等专题页面深入阅读。'
keywords:
  - lina-core
  - 主框架服务
  - 架构总览
