---
slug: '/docs/dev-skills'
title: 'Dev Skills'
description: 'A guide to LinaPro built-in AI skills — 13 purpose-built skills covering frontend design, frontend patterns, Git workflows, code review, E2E testing, performance auditing, and the OpenSpec workflow. Includes a quick-reference overview table and detailed usage instructions for each skill.'
keywords:
  - AI skills
  - dev skills
  - LinaPro skills
...
`LinaPro` ships a library of purpose-built AI skills under `.agents/skills/`. When you work inside the project directory with `Claude Code` or another compatible AI tool, these skills are loaded automatically — they give the AI the domain knowledge it needs to make framework-aware decisions in each specific scenario, so you never have to repeat project conventions by hand.
...
When an AI tool runs inside the project directory, it automatically loads all skills from `.agents/skills/`. Invoke a skill by describing your scenario in the conversation — the AI matches the context and applies the right skill. You can also trigger skills explicitly with a slash command (e.g. `/git-commit-push`).

For skills that have external dependencies, install the required tools before using them. See each skill's section below.
