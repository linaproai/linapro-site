...
hide_title: true
description: 'the read-only consumer interface design, Provider extension mechanism, capability degradation strategy, and its role in organization-scoped data permissions — helping plugin developers understand how to consume and extend organization capabilities.'
keywords:
...
- [TenantService](./7900-tenant.md) - Tenant capability complements organization capability, together forming the multi-tenant + organization data model
- [PluginStateService](./7600-plugin-state.md) - Uses `IsProviderEnabled` to determine whether the organization provider is available
- [SessionService](./7800-session.md) - Department names in session projections come from the organization capability
