...
Official source plugins live in `apps/lina-plugins/`, mounted as a `Git submodule`. When the submodule is not initialized, the core framework still runs in core framework-only mode. Pull official plugin content on demand with `git submodule update --init --recursive`.
...
- The core framework includes built-in tenant middleware and a `bizctx` tenant identity seam as a stable foundation
